...
public abstract class UIParallax : MonoBehaviour {
    ...
    protected void Parallaxing(Vector3 direction) {
        ...
        {
            var offset = direction * Time.deltaTime * ParallaxLayers[i].Intensity;
            ...
            var x = ParallaxLayers[i].Image.uvRect.x + offset.x;
            var y = ParallaxLayers[i].Image.uvRect.y + offset.y;
            if (x > 1 || x < -1) x = 0;
            if (y > 1 || y < -1) y = 0;
